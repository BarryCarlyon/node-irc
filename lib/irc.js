    ...
    self.addListener('raw', function(message) {
        ...
        switch (message.command) {
            ...
            case 'NOTICE':
                ...
                // twitch
                tags = message.tags || false;
                msg_id = '';
                if (message.tags) {
                    msg_id = message.tags.msg_id || '';
                }
                // twitch (and maintains compat)
                self.emit('notice', from, to, text, message, msg_id);

                // Specifics
                if (message.args[1] == 'Error logging in') {
                    console.log('Error Logging In');
                    process.exit();
                }
                if (message.args[1] == 'Improperly formatted auth') {
                    console.log('Improperly formatted auth');
                    process.exit();
                }
                if (message.args[1] == 'Login authentication failed') {
            ...
            case 'HOSTTARGET':
                ...
                var data = message.args[1].split(' ');
                if (data[0] == '-') {
                    self.emit('hostend', to, message);
                    self.emit('hosttarget', to, '', '', message);
                } else {
                    self.emit('hostchange', to, data[0], data[1], message);
                    self.emit('hosttarget', to, data[0], data[1], message);
                }
                break;

            // handler for bans and timeouts
            case 'CLEARCHAT':
                to = message.args[0];
                ...
                if (message.args[1]) {
                    self.emit('clearchat', to, message.args[1]);
                } else {
                    self.emit('clearedchat', to);
                }
                break;

            // USERSTATE is sent when joining a channel and every time you send a PRIVMSG to a channel
            case 'USERSTATE':
                to = message.args[0];
                if (!to) {
                    to = null;
                ...
                self.emit('userstate', to, tags);
                break;

            // Twitch IRC processes ocasionally need to be restarted. When this happens, clients that have
            // requested the IRCv3 twitch.tv/commands capability are issued a RECONNECT
            ...
            case 'USERNOTICE':
                ...
                self.emit('usernotice', to, text, tags, message);
                break;
            ...
            case 'GLOBALUSERSTATE':
                ...
                self.emit('globaluserstate', tags);
            ...
            case 'WHISPER':
                ...
                self.emit('whisper', from, to, text, tags, message);
                break;
...
Client.prototype._handleCTCP = function(from, to, text, type, message) {
    ...
    if (parts[0] === 'ACTION' && parts.length > 1) {
        // Twitch
        tags = message.tags || {};
        this.emit('action', from, to, parts.slice(1).join(' '), tags, message);
    }
